...
class TreeNode<T> implements Comparable<T>{
    ...
    /**
     * Function that compares wich node is bigger or if they are equal
     */
    @Override
...
public class AVLTree<T extends Comparable<T>> implements BinaryTreeInterface<T>{
    ...
    /**
     * Function that find a node in the tree structure, but if it isn't found, it returns the node pointer where it is supposed to be a child
     * 
     * @return pointer of the found node
     */
    public TreeNode<T> Find(T findKey, TreeNode<T> currentNode) {
        if (currentNode.getKey().compareTo(findKey) == 0) return currentNode;
    ...
    public void Insert(T key) {
        ...
        TreeNode<T> insertNode = Find(key, this.root);

        if (insertNode.getKey().compareTo(key) == 0) return;

        TreeNode<T> newNode = new TreeNode<T>(key, null, null, insertNode);
        
        if (insertNode.getKey().compareTo(key) < 0) insertNode.setRight(newNode);
        else insertNode.setLeft(newNode);
    ...
    public T Delete(TreeNode<T> deleteNode) {
        if (deleteNode.getLeft() != null){
            TreeNode<T> changeNode = Last(deleteNode);
            TreeNode<T> temporalNode = changeNode.getLeft();
            ...
            if (changeNode.getParent() != deleteNode){
                changeNode.setLeft(deleteNode.getLeft());
                changeNode.getLeft().setParent(changeNode);
                changeNode.getParent().setRight(temporalNode);
            }
            ...
            if (deleteNode.getParent() != null){
                if (deleteNode.getKey().compareTo((T) deleteNode.getParent().getKey()) > 0){
                    deleteNode.getParent().setRight(changeNode);
                }
                else if (deleteNode.getKey().compareTo((T) deleteNode.getParent().getKey()) < 0){
                    deleteNode.getParent().setLeft(changeNode);
                }
            }
    ...
    private void UpdateHeight(TreeNode<T> updateNode){
        ...
        if (updateNode.getLeft() == null) leftHeight = Height(updateNode.getLeft());
        if (updateNode.getRight() == null) rightHeight = Height(updateNode.getRight());
    ...
    private void Balance(TreeNode<T> balanceNode){
        ...
        if (balanceFactor >= 2) ExecuteRotations(balanceNode);
    ...
    private int CalculateBalanceFactor(TreeNode<T> node){
        ...
        if (node.getLeft() == null) leftHeight = Height(node.getLeft());
        if (node.getRight() == null) rightHeight = Height(node.getRight());
    ...
    private void ExecuteRotations(TreeNode<T> unbalancedNode){
        ...
        if (unbalancedNode.getLeft() == null) leftHeight = unbalancedNode.getLeft().getHeight();
        if (unbalancedNode.getRight() == null) rightHeight = unbalancedNode.getRight().getHeight();

        if (leftHeight > rightHeight){
            int left_leftHeight = 0, left_rightHeight = 0;

            if (unbalancedNode.getLeft().getLeft() == null) left_leftHeight = unbalancedNode.getLeft().getLeft().getHeight();
            if (unbalancedNode.getLeft().getRight() == null) left_rightHeight = unbalancedNode.getLeft().getRight().getHeight();

            if (left_leftHeight > left_rightHeight) unbalancedType = "Left_Left";
            else unbalancedType = "Right_Left";
        }
        else{
            int right_leftHeight = 0, right_rightHeight = 0;

            if (unbalancedNode.getRight().getLeft() == null) right_leftHeight = unbalancedNode.getRight().getLeft().getHeight();
            if (unbalancedNode.getRight().getRight() == null) right_rightHeight = unbalancedNode.getRight().getRight().getHeight();

            if (right_leftHeight > right_rightHeight) unbalancedType = "Left_Right";
            else unbalancedType = "Right_Right";
        }
    ...
    private void RotateLeft(TreeNode<T> pivotNode){
        ...
        //Cheking if the unbalanced node was at Right or Left of his parent, and placing the pointer in the right position
